...
class Trainer(object):
    def __init__(self, args):
        ...
        # models
        self.model = DeformableGrid(args, device).cuda()
    ...
    def build_data(self):
        ...
        self.composed_transforms_ts = transforms.Compose([
            tr.FixedResize({'image': self.resolution, 'gt': self.resolution},
                           flagvals={'image': cv2.INTER_LINEAR, 'gt': cv2.INTER_NEAREST}),
            tr.CropRandom(crop_size=self.resolution, keys=['image', 'gt'], drop_origin=True),
            tr.BilateralFiltering(['crop_image']),
            tr.ToTensor(),
            tr.Normalize('crop_image', mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
        ])

        self.valset = cityscapes_full.cityscapesFullLoader(split='train_val', transform=self.composed_transforms_ts)
        ...
        self.trainset = cityscapes_full.cityscapesFullLoader(split='train', transform=self.composed_transforms_ts)
