...
class ResNet(nn.Module):
    def __init__(self, block, layers, n_classes, input_channel=3, classifier="psp",
                 dilations=(2, 4), strides=(2, 2, 2, 1, 1), feature_channels=(64, 128, 256, 512)):
                 
        self.inplanes = 64
        self.classifier = classifier
        super(ResNet, self).__init__()
        self.conv1 = nn.Conv2d(input_channel, self.inplanes, kernel_size=7, stride=strides[0], padding=3,
                               bias=False)
    ...
    def forward(self, x, bbox=None):
        ...
        if self.layer5 is not None:
            #layer5_f, psp_out = self.layer5(layer4_f)
            layer5_f = self.layer5(layer4_f)
            return conv1_f, layer1_f, layer2_f, layer3_f, layer4_f, layer5_f #, psp_out
            #return psp_out
    ...
    def reload(self, path, strict=False):
        self.load_state_dict(torch.load(path)['state_dict'],
            strict=strict)
def resnet101(n_classes, input_channel=3, classifier="atrous",
              #dilations=(2, 4), strides=(1, 2, 1, 2, 2)):
              dilations=(2, 4), strides=(2, 2, 2, 1, 1)):
    """Constructs a ResNet-101 model.
    """
    model = ResNet(Bottleneck, [3, 4, 23, 3], n_classes=n_classes, input_channel=input_channel,
